...
for i in $(seq 1 30);do
   ...
   ssh_pid=`ps -ef |grep $md5_str |grep -v grep |grep -v trace_session.sh|grep -v sshpass |awk '{print $2}'`
   ...
   if [ "$ssh_pid" = "" ];then
      ...
      continue
        ...
        echo 123 | sudo -S /usr/bin/strace -ttt -f -p $ssh_pid -o "$today_audit_dir/$md5_str.log"
      break
   fi;
